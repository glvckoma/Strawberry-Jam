   ...
   public class ECardCreation
   {
      ...
      private function onSentResponseCallback(param1:int) : void
      {
         ...
         if(param1 < 1)
         {
            if(param1 == -1)
            {
               _loc3_ = LocalizationManager.translateIdOnly(24507);
            }
            else if(param1 == -2)
            {
               _loc2_ = gMainFrame.userInfo.getUserInfoByUserName(_toUserName);
               _loc3_ = _loc2_ ? LocalizationManager.translateIdAndInsertOnly(25038,_loc2_.getModeratedUserName()) : LocalizationManager.translateIdOnly(14717);
            }
            else
            {
               _loc3_ = LocalizationManager.translateIdOnly(14717);
            }
            new SBOkPopup(_guiLayer,_loc3_);
         }
