package createAccountFlow
{
   ...
   public class CreateAccount extends MovieClip implements INewAvatar
   {
      ...
      {
         var _loc4_:RegExp = new RegExp(SbiConstants.PASSWORD_REGEX[0]);
         var _loc5_:RegExp = new RegExp(SbiConstants.PASSWORD_REGEX[1]);
         var _loc6_:RegExp = new RegExp(SbiConstants.PASSWORD_REGEX[2]);
         if(param1 && param1 != "" && param1.length >= 12)
         {
            ...
            else if(!(_loc4_.test(param1) && Boolean(_loc5_.test(param1)) && Boolean(_loc6_.test(param1))))
            {
               CreateAccountGui.showNewPasswordBadMsg(-101);
               param3(false);
               SBTracker.trackPageview("/login/usernameValidation/#passwordInvalid",0,1);
            }
            else
