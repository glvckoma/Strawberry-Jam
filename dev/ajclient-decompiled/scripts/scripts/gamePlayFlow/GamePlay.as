package gamePlayFlow
{
   ...
   public class GamePlay extends MovieClip
   {
      ...
      private var _lastAntiAfkSendTime:int = 0; 
      private const ANTI_AFK_INTERVAL:int = 387000;
      ...
      public function heartbeat(param1:Event) : void
      {
         ...
         if (gMainFrame && gMainFrame.server && gMainFrame.server.isConnected)
         {
            if ((_loc2_ - _lastAntiAfkSendTime) > ANTI_AFK_INTERVAL)
            {
               DebugUtility.debugTrace("[Anti-AFK] Sending ka and rc packets. Current time: " + _loc2_);
               gMainFrame.server.setXtObject_Str("ka", [], gMainFrame.server.isWorldZone);
               gMainFrame.server.setXtObject_Str("rc", [], true, false, false);
               _lastAntiAfkSendTime = _loc2_;
            }
         }
      }
